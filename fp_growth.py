...
def Load_data(filename):
    ...
    for i in range(0, len(content)):
        line = content[i].split()[3:]
        line = [eval(v) for v in line]
        Transaction.append(set(line))
    return Transaction
...
def find_frequent_itemsets(transactions, support_ratio, include_support=False):
    ...
    for transaction in transactions:
        for item in transaction:
            items[item] += 1
    minimum_support = len(transactions)*support_ratio
    # 频繁1项集筛选
...
def savetxt(items):
    ...
    for item, support in sorted(items, key=lambda x: x[1],reverse=True):
        output += str(round(support/len(Transaction)*100,1))+'\t'+str(set(item))+'\n'
    f.write(output)
    f.close()
...
if __name__ == '__main__':
    ...
    Transaction = Load_data(options.inputFile)
    initSet = create_initialset(Transaction)
    start = time.time()

    frequent_itemsets = find_frequent_itemsets(Transaction, support_ratio=options.minSup, include_support=True)
    ...
    #     print(str(set(itemset)) + ' ' + str(round(support/len(Transaction)*100,1)))
    savetxt(result)
